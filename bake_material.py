...
def materialAsJSON(o, path, resolution, bake_albedo, bake_normal, bake_rough, bake_metal, bake_ao, bake_height):
    ...
    for i, s in enumerate(o.material_slots):
        ...
        for n in node_tree.nodes:
            if n.type == 'BSDF_PRINCIPLED':
                ...
                material.node_tree.interface_update(bpy.context)
                material.node_tree.preview_ensure()
                material.update_tag(refresh={'TIME'})
    ...
    # 4.3   Bake the texture
    # 4.3.1 Bake type = diffuse
    # 4.3.2 Contributaions = Color
    # 4.3.3 Run the bake operation
